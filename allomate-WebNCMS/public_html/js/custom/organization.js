...
$(document).ready(function () {
    ...
    $(".save_form").on('click', function () {
        ...
        var url_clutch      = $('.url_clutch').val();
        var url_dribble     = $('.url_dribble').val();
        var url_behance     = $('.url_behance').val();

        if (urlReg.test(url_clutch) == false && url_clutch != '') {
            url_valitdation = true;
            error_url = "Clutch URL is Invalid"
        }
        if (urlReg.test(url_dribble) == false && url_dribble != '') {
            url_valitdation = true;
            error_url = "Dribble URL is Invalid"
        }
        if (urlReg.test(url_behance) == false && url_behance != '') {
            url_valitdation = true;
            error_url = "Behance URL is Invalid"
        }
        ...
        $('#form').ajaxSubmit({
            ...
            success: function (response) {
                ...
                if (response.status == "error") {
                    ...
                    currentRef.text('Saved');
                    $('#notifDiv').fadeIn();
                    $('#notifDiv').css('background', 'Green');
                    $('#notifDiv').text('Organization Details has been Addedd Successfully');
                    setTimeout(() => {
                        $('#notifDiv').fadeOut();
                    }, 1500);
                    setTimeout(() => {
    ...
    $(document).on('click', '.openDataSidebarForUpdateLocation', function () {
        ...
        $.ajax({
            type: 'GET',
            url: '/admin/get-location-form/' + id,
            success: function (response) {
                $('#countries_2').empty();
    ...
    $(document).on('click', '#savelocationBtn', function () {
        ...
        $('#location_form').ajaxSubmit({
            url: '/admin/organization-location/store',
                    ...
                    $('.location_table tbody').append(`
                            <tr>
                                ...
                                <td><a id="${element['id']}"  class="btn btn-default btn-line m-b openDataSidebarForUpdateLocation">Edit</a>
                                  <a href="javascript:void(0);"   id="${element['id']}" class="btn btn-default red-bg location_delete"  >Delete</a>
                                </td>
                            </tr> `)
                });
    ...
    $(document).on('click', '.location_delete', function () {
        ...
        swal({
            ...
            if (willDelete) {
                $.ajax({
                    url: '/admin/location-delete/' + location_id,
                    type: 'POST',
                    headers: {

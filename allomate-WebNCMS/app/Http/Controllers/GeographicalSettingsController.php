...
class GeographicalSettingsController extends AccessRightsAuth
{
    public function geographicalsetting(){
        return view('admin.geographicalSettings.geographicalsetting');
    }
    ...
    public function getAllStates(){
        try{
            $states = State::where('country_id',39)->selectRaw("id,name")->get();
            return response()->json(['states'=>$states,'msg'=>'success']);
        }catch (\Illuminate\Database\QueryException $ex) {
            return response()->json($ex,422);
        }
    }
    public function getCitiesAgainstState($id){
        $cities     =   DB::Select("SELECT id,name,state_id,country_id From cities Where state_id = $id ");
        return response()->JSON([
            'msg'      =>  'success',
            'cities'   =>  $cities
        ]);

...
class Admin extends AccessRightsAuth
{
    ...
    {
        $icon = "";
        $parentMod = FacadesDB::table('controllers')
            ->where('parent_module', $request->parent)
            ->orderBy('id', 'desc')
            ->first();
        if ($request->hasFile('icon')) {
            ...
            $public_path            =   public_path('/menuicon');
            $path                   =   $request->file('icon')->move($public_path, $iconPath);
            $icon                   =   $iconPath;
        }
        if ($request->item_id) {
    ...
    public function SaveParentMod(Request $request)
    {
        ...
        if ($request->hasFile('parent_icon')) {
            ...
            $iconPath           =   str_replace(' ', '_', $fileNameOnly) . '-' . rand() . '_' . time() . '.' . $extension;
            $public_path        =   public_path('/menuicon');
            $path               =   $request->file('parent_icon')->move($public_path, $iconPath);

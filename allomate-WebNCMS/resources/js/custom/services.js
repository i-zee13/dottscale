...
$(document).ready(function() {
    ...
    if (action == "SecondaryServices") {
        fetchSecondaryServices();
    } else if(action == "sub-secondary-service"){
        fetchSubSecondaryServices();
    ...
    $(document).on('click', '.update-blog-cate', function() {
        ...
        $.ajax({
            type: 'GET',
            url: `/admin/get-blog-category/${id}`,
        ...
        $('#saveMainCatForm').ajaxSubmit({
            ...
            success: function(response) { 
                if (response.status == "success") {
                    ...
                    $('#notifDiv').text('Service have been updated successfully');
                    $('#notifDiv').fadeIn();
                    ...
                    if(response.msg == "duplicate") {
                        $('#notifDiv').fadeIn();
                        $('#notifDiv').css('background', 'red');
                        $('#notifDiv').text('Service Already Exist');
                        setTimeout(() => {
                            $('#notifDiv').fadeOut();
                        }, 3000);
                        $('#saveMainCat').removeAttr('disabled');
...
function fetchMainCategories() {
    $.ajax({
        type: 'GET',
        url: '/admin/get-blogs-categories',
        success: function(response) {
            $('.body').empty();
...
$(document).on('click', '.cate_delete', function () {
    ...
    swal({
        ...
        if (willDelete) {
            $.ajax({
                url: '/admin/delete-blog-category/'+cat_id,
                type: 'POST',
                headers: {

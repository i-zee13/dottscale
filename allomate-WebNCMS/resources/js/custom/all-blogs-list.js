...
function all_blogs_list(){
    ...
    $.ajax({
        type    :   'GET',
        url     :   '/admin/all-blogs-list',
        success :   function(response){
            $('.all_blogs').append('<table class="table table-hover dt-responsive nowrap BlogsListTable" style="width:100%;"><thead><tr><th>S.No</th><th>Date</th><th>Blog Title</th><th>Status</th><th>Action</th></tr></thead><tbody></tbody></table>');
            ...
            response.all_blogs.forEach((element, key) => {
                ...
                $('.BlogsListTable tbody').append(`
                    <tr>
                        <td>${key + 1}</td>
                        <td>${element['date'] ? element['date'] : '00-00-0000'}</td>
                        <td>${element['title']}</td>
                        <td>${element.published == 1? 'Published':'Not Published'}
                        <td>
                            <a href="/admin/edit-blog/${element['id']}" id="${element['id']}" class="btn btn-default btn-line edit_blog"> Edit </a>
                            <button id="${element['id']}" class="btn btn-default red-bg delete_blog">Delete</button>
                            <button id="${element['id']}" class="btn btn-default status_change_blog" data-value="${element.published}">${element.published==1 ? 'Unpublish' : 'Publish'}</button>
                        </td>
                    </tr>`);
            });
...
//             success :   function (response) {
//                 window.location = '/admin/edit-blog';

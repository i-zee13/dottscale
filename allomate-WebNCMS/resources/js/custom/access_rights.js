...
$(document).ready(function () {
    ...
    $(document).on('click', '.openDataSidebarForAddingAccessRights', function () {
        ...
        var all_Investors_rights    =   $('.employee_list').val();
        var All_Investors           =   [];
        ...
        if (All_Investors) {
            ...
            All_Investors = All_Investors.filter(employee => !alreadyExistingRights.some(right => right.employee_id === employee.id));
            console.log(All_Investors);
            ...
            All_Investors.forEach(employee => {
                var option = $('<option>', {
                    value: employee.id,
...
function fetchAccessRights() {
    $.ajax({
        ...
        success: function (response) {
            ...
            var sNo = 1;
            response.forEach(element => {
                $('.rightsTable tbody').append('<tr><td>' + sNo++ + '</td><td>' + element['name'] + '</td><td>' + element['total_rights'] + '</td><td><button id="' + element['team_id'] + '" class="btn btn-default btn-line openDataSidebarForUpdateAccessRights">Update</button><button type="button" id="' + element['team_id'] + '" class="btn btn-default red-bg deleteAccessRight" title="Revoke">Revoke</button></td></tr>');
                alreadyExistingRights.push({ 'employee_id': element['team_id'] });
            });
            $('#tblLoader').hide();
            $('.body').fadeIn();

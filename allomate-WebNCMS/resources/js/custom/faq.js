...
$(document).on('click', '.add_faqs', function () {
    ...
    $('select[name="faq_type"]').val('').trigger('change');
    $('select[name="page_id"]').val('').trigger('change');
...
$('#faq_type').change(function () {
    ...
    if (type_id == 2) {
        $('.page_row').fadeIn();
        $('#page_id').addClass('faq-required');
    } else {
        $('.page-info').removeClass('faq-required');
        $('.page_row').hide();
...
$(document).on('click', '#save-faq', function () {
    ...
    if (!faq_details && !faq_details.trim()) {
        dirty = true;
    }
    if (dirty) {
        $('#notifDiv').fadeIn();
...
$(document).on('click', '.edit_faq', function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('select[name="page_id"]').val(response.faqs_result.page_id).trigger('change');
            $('input[name="faq_question"]').focus();
        ...
        success: function (response) {
            ...
            response.all_faqs.forEach((element, key) => {
                $('.FaqsListTable tbody').append(`
                    <tr>
                        <td>${key + 1}</td>
                        <td>${element['faq_type'] == 1 ? 'General' : element['faq_type'] == 2 ? 'Page Related' : 'NA'}</td>

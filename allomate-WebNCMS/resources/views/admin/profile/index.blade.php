...
<div class="row">
    <div class="col-lg-4 col-12 mb-30">
        <div class="card cp_user-l top_border">
            <div class="body">
                <div class="_cut-img mt-30">
                    <img src="{{ Auth::user()->picture ? URL::to(Auth::user()->picture) : "/admin/images/avatar.svg"}}"
                        alt="" />
                    <div class="nam-title">{{ (Auth::user()->name ? Auth::user()->name : "NA") }}</div>
                </div>
                <div class="con_info lineHeight30">
                    <p>
                        <i class="fa fa-briefcase"></i><strong>{{ $designation->designation ?$designation->designation:'NA' }}</strong>
                    </p>
                    <p>
                        <i class="fa fa-phone-square"></i><strong>{{ (Auth::user()->phone ? Auth::user()->phone : "NA") }}</strong>
                    </p>
                    <p>
                        <i class="fa fa-envelope-square"></i>{{ (Auth::user()->email ? Auth::user()->email : "NA") }}
                    </p>
                    <p>
                        <i class="fa fa-globe"></i>{{ (Auth::user()->address ? Auth::user()->address : "NA") }}
                    </p>
                </div>
            </div>
        </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 p-col-L">
                                    <div class="form-group">
                                        ...
                                        <p>{{ (Auth::user()->name ? Auth::user()->name : "NA") }}</p>
                                    </div>
                                </div>
                                <div class="col-md-6 p-col-R">
                                    <div class="form-group">
                                        <label class="control-label mb-10">Phone No</label>
                                        <p>{{ (Auth::user()->phone ? Auth::user()->phone : "NA") }}</p>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6 p-col-L">
                                    <div class="form-group">
                                        ...
                                        <p>{{ (Auth::user()->email ? Auth::user()->email : "NA") }}</p>
                                    </div>
                                </div>
                                <div class="col-md-6 p-col-R">
                                    <div class="form-group">
                                        <label class="control-label mb-10">SIN No</label>
                                        <p>{{ (Auth::user()->cnic ? Auth::user()->cnic : "NA") }}</p>
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                ...
                                <div class="col-md-12 ">
                                    <div class="form-group">
                                        ...
                                        <p>{{ (Auth::user()->address ? Auth::user()->address : "NA") }}
                                        </p>
                                    </div>
                                </div>
                            </div> 
                                    ...
                                    <div>
                                        <div class="col-md-6">
                                            <div class="form-wrap up_h" style="width:100px;">
                                                ...
                                                <input type="file" id="input-file-now" class="dropify profile-pic" name="employeePicture"
                                                    data-default-file="{{ Auth::user()->picture ? URL::to(Auth::user()->picture) : '' }}" accept="image/*"/>
                                            </div>
                                            <button type="button" class="btn btn-primary"
                                                id="save-user-profile-pic">Save</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
...
@endsection
@push('js')
<script src="{{mix('js/custom/profile.js')}}"></script>
@endpush